...
@section('header', 'Add Restaurant')

@section('content')

<div class="max-w-2xl">
    <form method="POST" action="{{ route('super_admin.restaurants.store') }}">
        @csrf
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="address" rows="2"
                        class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent">{{ old('address') }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="order_method" required
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                            <option value="digital" {{ old('order_method') == 'digital' ? 'selected' : '' }}>Digital</option>
                            <option value="manual" {{ old('order_method') == 'manual' ? 'selected' : '' }}>Manual (Ticket)</option>
                        </select>
                    </div>
                </div>
